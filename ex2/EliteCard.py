...
class EliteCard(Card, Combatable, Magical):
    def __init__(
        ...
        if not isinstance(damage, int) or damage <= 0:
            raise Exception(
                ...
                "The damage argument must be a positive non zero integer"
            )

        if not isinstance(damage_blocked, int) or damage_blocked < 0:
            raise Exception(
                "[INITIALIZATION ERROR]: "
                "The damage_blocked argument must be a positive integer"
            )

        if not isinstance(health, int) or health <= 0:
            raise Exception(
                "[INITIALIZATION ERROR]: "
                "The health argument must be a positive non zero integer"
            )
    ...
    def play(self, game_state: dict) -> dict:
        ...
        if "battlefield" not in game_state:
            raise ValueError(
                "[LOGIC ERROR]: missing 'battlefield' key in "
                "game_state argument!"
            )
    ...
    def attack(self, target) -> dict:
        if not isinstance(target, Card):
            raise TypeError(
                "[LOGIC ERROR]: The target argument is not a Card instance"
            )

        if not hasattr(target, "health"):
            raise AttributeError(
                "[LOGIC ERROR]: The target can't be attacked, "
                "it has no health attribute"
            )
    ...
    def defend(self, incoming_damage: int) -> dict:
        if not isinstance(incoming_damage, int) or incoming_damage <= 0:
            raise Exception(
                "[LOGIC ERROR]: "
                "The incoming_damage argument must be a positive "
                "non zero integer"
            )
    ...
    def channel_mana(self, amount: int) -> dict:
        if not isinstance(amount, int) or amount <= 0:
            raise Exception(
                "[LOGIC ERROR]: "
                "The amount argument must be a positive non zero integer"
            )
