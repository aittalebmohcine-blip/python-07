...
class SpellCard(Card):
    def __init__(self, name: str, cost: int, rarity: str, effect_type: str) -> None:
        ...
        if not isinstance(effect_type, str):
            raise TypeError(
                "[INITIALIZATION ERROR]: The effect_type argument must be a string type."
            )
    ...
    def play(self, game_state: dict) -> dict:
        ...
        if "mana" not in game_state:
            raise KeyError("[LOGIC ERROR]: missing 'mana' key in game_state!")
    ...
    def resolve_effect(self, targets: list) -> dict:
        if not isinstance(targets, list):
            raise TypeError(
                "[LOGIC ERROR]: The targets argument must be a list of instances of the CreatureCard class"
            )
        ...
        return {
            ...
            "targets_effected": [target.name for target in targets],
        }
