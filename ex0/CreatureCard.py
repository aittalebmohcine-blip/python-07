...
class CreatureCard(Card):
    def __init__(
        ...
        if not isinstance(attack, int) or attack <= 0:
            raise ValueError(
                "[INITIALIZATION ERROR]: The attack argument must be a positive intiger"
            )
        self.attack = attack
        if not isinstance(health, int) or health <= 0:
            raise ValueError(
                "[INITIALIZATION ERROR]: The health argument must be a positive non zero intiger"
            )
        self.health = health
    ...
    def attack_target(self, target) -> dict:
        if not isinstance(target, CreatureCard):
            raise TypeError(
                "[LOGIC ERROR]: The target argument should be an isinstance of the CreatureCard."
            )
        return {
            "attacker": self.name,
            "target": target.name,
